...
//
#ifndef SPEKTRUM_TELEMETRY_DEFS_H
#define	SPEKTRUM_TELEMETRY_DEFS_H
...
typedef struct {
	...
	INT16		delta_0500ms;				// change in altitude last 500ms, 0.1m/s increments
	INT16		delta_1000ms;				// change in altitude last 1.0 seconds, 0.1m/s increments
	INT16		delta_1500ms;				// change in altitude last 1.5 seconds, 0.1m/s increments
	INT16		delta_2000ms;				// change in altitude last 2.0 seconds, 0.1m/s increments
	INT16		delta_3000ms;				// change in altitude last 3.0 seconds, 0.1m/s increments
} STRU_TELE_VARIO_S;
...
typedef struct {
	...
	UINT8		gainPitch;				// Pitch Gain
	UINT8		gainYaw;				// Yaw Gain
	INT16		attRoll;				// Roll Attitude, 0.1degree, RHR
	INT8		attPitch;				// Pitch Attitude
	INT8		attYaw;					// Yaw Attitude
	UINT16		spare;
} STRU_TELE_ALPHA6;
...
typedef struct {
	...
	UINT8		RFmode;					// 4fps=0, 50fps, 150Hz
	UINT8		upPower;				// 0mW=0, 10mW, 25mW, 100mW, 500mW, 1000mW, 2000mW
	UINT8		downlink;				// dBm * -1
	UINT8		qualityDown;				// %
	INT8		SNRdown;				// dB
} STRU_TELE_XF_QOS;
